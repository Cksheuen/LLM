...
export default function LLMAnswer({
    content, streamChats, setScrollBar, completeAddNewChat
}: LLMAnswerProps) {
    const corner_btns = [
        {
            name: "Copy",
    ...
    function goOn() {
        if (times.current === 0) {
            ...
            if (streamChats![index.current].event === 'conversation.message.completed' &&
                (streamChats![index.current].data as MessageObj).type === 'answer') {
                completeAddNewChat && completeAddNewChat(streamChats![index.current].data as MessageObj)
            }
    ...
    return (
        <div ref={container} className="dialog-box w-full max-w-3xl">
            ...
            {
                streamOver && (
                    <div className="flex items-center justify-center text-2.5 gap-2 absolute bottom--3 right--3
            bg-gray-5 p-2 rd-1 shadow-gray-4 shadow animate-fade-in duration-100">
                        {
                            corner_btns.map((btn) => {
                                return (
                                    <div key={btn.name} className="flex items-center justify-center gap-1
                            cursor-pointer">
